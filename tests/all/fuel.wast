...
;; The bulk operations below request enough units (cost > 128, the
;; `SMALL_BULK_OP_COST` fast-path threshold) that the size-proportional
;; "variable" fuel is deferred and charged after the op runs, rather than up
;; front like the smaller ops above. The total billed must still match the
;; flat formula: this asserts the deferred charge bills the right amount.
(assert_fuel 207
  (module
    (memory 1)
    (func $f
