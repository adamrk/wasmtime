        ...
        + 'static,
{
    ...
    // Store initially holds `None`, but is expected to be `Some` by the time
    // the imported function is actually called.
    ...
    if declarations.options.host_async {
        linker
            ...
            .func_wrap_concurrent(IMPORT_FUNCTION, |a, params| {
                Box::pin(async move {
                    a.with2(|mut cx| host_function::<P, R>(cx.as_context_mut(), params))
                        .await
                })
            })
            .unwrap();
        ...
        linker
            ...
            .func_new_concurrent(IMPORT_FUNCTION, {
                move |cx: &Accessor<_, _>, _, params: &[Val], results: &mut [Val]| {
                    Box::pin(async move {
                        cx.with2(|mut store| host_function(store.as_context_mut(), params, results))
                            .await
                    })
                }
            })
