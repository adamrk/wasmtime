...
pub async fn async_closed_stream() -> Result<()> {
    ...
    store
        .run_concurrent(async move |accessor| {
            ...
            accessor
                .with2(move |store| stream.pipe(store, PipeConsumer::new(tx)))
                .await?;
...
async fn test_async_short_reads(delay: bool) -> Result<()> {
    ...
    store
        .run_concurrent(async |store| {
            ...
            let stream = store
                .with2(|store| StreamReader::new(store, VecProducer::new(things, delay)))
                .await?;
            ...
            store
                .with2(|store| stream.pipe(store, OneAtATime::new(received_things.clone(), delay)))
                .await?;
